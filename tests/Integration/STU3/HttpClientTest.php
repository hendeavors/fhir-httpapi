...
class HttpClientTest extends TestCase
{
    ...
    public function guzzleHttpGetRequest()
    {
        $mock = new MockHandler([
            ...
            new Response(200, ['X-Foo' => 'Bar'], 'Referral Request'),
            new Response(200, ['X-Foo' => 'Bar'], 'Procedure Request'),
        ...
        $response = $clinical->referralRequest(new Request('GET', 'http://fhirserver/careteam', [
            'Accept' => 'application/json',
            'Authorization' => 'Bearer token'
        ]));

        $this->assertSame('Referral Request', (string)$response->getBody());

        $response = $clinical->procedureRequest(new Request('GET', 'http://fhirserver/careteam', [
            'Accept' => 'application/json',
            'Authorization' => 'Bearer token'
        ]));

        $this->assertSame('Procedure Request', (string)$response->getBody());
